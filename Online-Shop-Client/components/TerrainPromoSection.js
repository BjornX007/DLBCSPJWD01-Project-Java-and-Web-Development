 ...
 * --------------------------------------------------
 * Purpose:
 * - Visually promotes bike categories (terrains)
 * - Highlights current sale / promotional deals
 *
 * Features:
 * - Responsive grid layout
 * - Hover animations and image scaling
 * - Gradient overlays for text readability
 * - Mock data prepared for future API replacement
 */
...
export default function TerrainPromoSection() {
  /**
   * State: bike terrain categories
   */
  const [terrains, setTerrains] = useState([]);

  /**
   * State: sale / promotional items
   */
  const [saleItems, setSaleItems] = useState([]);

  /**
   * Load mock data
   * (Can later be replaced with API calls)
   */
  useEffect(() => {
    setTerrains([
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
          {terrains.map((terrain) => (
            <a
              key={terrain.id}
              href={`/category/${terrain.slug}`}
              className="group block relative overflow-hidden rounded-2xl bg-white border border-gray-200 shadow-lg
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
          {saleItems.map((item) => (
            <a
              key={item.id}
              href={`/sale/${item.id}`}
              className="group block relative overflow-hidden rounded-2xl bg-white border border-red-200
                         shadow-md hover:shadow-xl transform hover:-translate-y-1 transition-all"
            >
              {/* Sale Image */}
              <img
                src={item.imageUrl}
